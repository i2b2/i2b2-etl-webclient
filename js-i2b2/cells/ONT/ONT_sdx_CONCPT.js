...
i2b2.sdx.TypeControllers.CONCPT.RenderHTML= function(sdxData, options, targetDiv) {
	...
	if (env.derivedConcept === true) {
		if (i2b2.PM.model.userRoles.includes('POPULATION_FACT_VIEWER') && i2b2.sdx.TypeControllers.MConcept.errorStatusCode === 200) {
			i2b2.sdx.TypeControllers.MConcept.derivedIcon_RenderHTML(sdxData);
		}
	}
	//===========Commented for M-release===========//
...
// This would ensure removal of the onmouseover call in a cross-browser way
		...
		var conceptDetails = {
			conceptPath: sdxData.origData.key,
			conceptName: sdxData.origData.name
		};
		//===========Commented for M-release===========//
		if (i2b2.ONT.view.mInfo.dCTempDialogActive) {
			var sDD = ' draggable="true" style="cursor: pointer;" ondragstart="i2b2.sdx.TypeControllers.MConcept.handleOnDragTempConstraint(event,\'' + encodeURI(JSON.stringify(conceptDetails)) + '\')" onmouseout="i2b2.sdx.TypeControllers.CONCPT.RemoveHighlight(\'' + id + '\')" ';
		} 
	...
	if (Object.isString(options.cssClass)) {
		...
		retHtml += ' class= "sdxDefaultCONCPT" id="' + id + '" ';
	}
	retHtml += sImgEvents;
	retHtml += '>';
	retHtml += '<IMG class="defaultIcon" src="'+render.icon+'"/>';
	if (sdxData.origData.status !== undefined) {
		if (sdxData.origData.status == 'COMPLETED'){
			retHtml += '<IMG class="addIcon" src="js-i2b2/cells/ONT/assets/icons8-settings-14-green.png"/>';
		} else if (sdxData.origData.status == 'ERROR') {
			retHtml += '<IMG class="addIcon" src="js-i2b2/cells/ONT/assets/icons8-settings-14-red.png"/>';
		} else if (sdxData.origData.status == 'PROCESSING') {
			retHtml += '<IMG class="addIcon" src="js-i2b2/cells/ONT/assets/icons8-settings-14.png"/>';
		} else {
			retHtml += '<IMG class="addIcon" src="js-i2b2/cells/ONT/assets/icons8-settings-14.png"/>';
		}
	}
	...
	if (env.derivedConcept === true) {
		setTimeout(function() {
			i2b2.sdx.TypeControllers.MConcept.runCode(sdxData, render);
		}, 400);
	}
...
i2b2.sdx.TypeControllers.CONCPT.AppendTreeNode = function(yuiTree, yuiRootNode, sdxDataPack, callbackLoader) {    
	...
	if (env.derivedConcept === true) {
		if (i2b2.PM.model.userRoles.includes('POPULATION_FACT_VIEWER') && i2b2.sdx.TypeControllers.MConcept.errorStatusCode === 200) {
			i2b2.sdx.TypeControllers.MConcept.AppendTreeNode(tmpNode);
		}
	}
	//===========Commented for M-release===========//
...
//		TODO: REFACTOR THIS! (Roll into COMM message sniffer?)
		...
		for(var i=0; i<1*c.length; i++) {
			...
			if (env.derivedConcept === true) {
				if (i2b2.PM.model.userRoles.includes('POPULATION_FACT_VIEWER') && i2b2.sdx.TypeControllers.MConcept.errorStatusCode === 200) {
					i2b2.sdx.TypeControllers.MConcept.LoadConcepts(sdxDataNode);
				}
			}
			//===========Commented for M-release===========//
			...
			if (modifier) {				/* jgk 0819 - this code seems no longer used? */
				var renderOptions = {
						...
						click: "i2b2.ONT.view.info.SetKey('"+encodeURI(sdxDataNode.sdxInfo.sdxKeyValue)+"', '"+sdxDataNode.origData.derivedCode+"')",
						icon: {
			...
			if (env.derivedConcept === true) {
				i2b2.sdx.TypeControllers.MConcept.LoadConcepts_addDerivedIcon(sdxDataNode, sdxRenderData);
			}
			//===========Commented for next-release===========//
...
i2b2.sdx.TypeControllers.CONCPT.MakeObject = function(c, modifier, cl_options, origData, objectType) {
	...
	if (env.derivedConcept === true) {
		if(i2b2.PM.model.userRoles.includes('POPULATION_FACT_VIEWER') && i2b2.sdx.TypeControllers.MConcept.errorStatusCode === 200){
			i2b2.sdx.TypeControllers.MConcept.MakeObject(o);
		}
	}
	//===========Commented for next-release===========//
