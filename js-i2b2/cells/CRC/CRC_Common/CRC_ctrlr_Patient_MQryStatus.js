 ...
 * @namespace	i2b2.CRC.ctrlr.MQryStatusPatient
 ...
 */
 ...
 const M_QRY_PATIENT_STATUS_BASE_URL = window.location.origin;

 i2b2.CRC.ctrlr.MQryStatusPatient = function(dispDIV) { 
    this.dispDIV = dispDIV; 
};
 
 i2b2.CRC.ctrlr.MQryStatusPatient._GetTitle = function(resultType, oRecord, oXML) {
     var title = "";
     switch (resultType) {
 ...
 i2b2.CRC.ctrlr.MQryStatusPatient.prototype = function() {
        ...
        scopedCallbackQRS.callback = function(results) {
            if (results.error) {
                ...
                for (var i=0; i<l; i++) {
                    ...
                    if (!rec.title) {
                        rec.title = i2b2.CRC.ctrlr.MQryStatusPatient._GetTitle(rec.QRS_Type, rec, temp);
                    }				
                    self.QRS[qrs_id] = rec;
                }
            ...
            // force a redraw
            i2b2.CRC.ctrlr.currentQueryStatus.patientSetrefreshStatus();
        }
    ...
    function patientSet_private_refresh_status() {
            ...
            if (results.error) {
                ...
                if (i2b2.CRC.view.graphs.bisGTIE8) {
                    ...
                    if (sCompiledResultsTest.includes("breakdown")) {
                        ...
                        document.getElementById('createPatientSetDialog_mask').style.display = 'none';
                        onLoadPatientSetForm();
                        getQueryMasterDataForPatientSet();
                    }
   ...
   async function getQueryMasterDataForPatientSet() {
        ...
        try {
            const response = await fetch(M_QRY_PATIENT_STATUS_BASE_URL + "/etl/patientSetQueryMaster?name=" + queryName, requestOptions);
            const responseData = await response.json();
            document.getElementById('patientSetCode').value = responseData.name;
            document.getElementById('patientSetFactQuery').value = responseData.generatedSql;
            return responseData;
        }
        catch (error) {
   ...
   async function addPatientSet() {
       let codeValue = document.getElementById('patientSetCode').value;
       let codeIndex = (codeValue.substring(codeValue.indexOf(")")+1)).trim();
       let cPath = i2b2.CRC.view.displayTabs.formatPathWithSlash(document.getElementById('patientSetPath').value);
       let derivedConceptPath = i2b2.CRC.view.displayTabs.modifyPathWithTitle(cPath, 'PatientSet');
       var rawData = JSON.stringify({
           "description": document.getElementById('patientSetDescription').value,
           "path": derivedConceptPath,
           "code": codeIndex,
           "factQuery": document.getElementById('patientSetFactQuery').value,
           "type": document.getElementById("patientSetType1").checked ? 'TEXTUAL' : 'NUMERIC',
           "definitionType": "PATIENTSET"
       })
       ...
       try {
           const response = await fetch(M_QRY_PATIENT_STATUS_BASE_URL + "/etl/patient-set", requestOptions);
           const responseData = await response.json();
    ...
    function onLoadPatientSetForm() {
        ...
        this._loadPatientSetDialog(handleSubmit);
    }

    // DIALOG FOR DERIVED CONCEPT FORM START
    this._loadPatientSetDialog = function(handleSubmit) {
        ...
        var handleSave = function() {
            i2b2.CRC.view.createPatientSetDialogPopUp.submitterFunction();
        };

        i2b2.CRC.view.createPatientSetDialogPopUp = new YAHOO.widget.SimpleDialog("createPatientSetDialog", {
            width: "500px",
            fixedcenter: true,
        ...
        $('createPatientSetDialog').show(); 
          
        i2b2.CRC.view.createPatientSetDialogPopUp.validate = function(){
           // now process the form data
           var msgError = '';
           var path = $('createPatientSetDialog').select('INPUT#patientSetPath')[0];
           var code = $('createPatientSetDialog').select('INPUT#patientSetCode')[0];
        ...
        i2b2.CRC.view.createPatientSetDialogPopUp.render(document.body);

        // manage the event handler for submit
        delete i2b2.CRC.view.createPatientSetDialogPopUp.submitterFunction;
        i2b2.CRC.view.createPatientSetDialogPopUp.submitterFunction = handleSubmit;
        // display the dialoge
        i2b2.CRC.view.createPatientSetDialogPopUp.center();
        i2b2.CRC.view.createPatientSetDialogPopUp.show();
    }
    ...
    function patientSetPrivate_startQuery() {
        var self = i2b2.CRC.ctrlr.currentQueryStatus;
                    ...
                    if (false && (self.QI.status == "INCOMPLETE" || self.QI.status == "COMPLETED" || self.QI.status == "ERROR")) {
                        ...
                        for (var i=0; i<l; i++) {
                            try {
                                ...
                                // set the proper title if it was not already set
                                if (!temp.title) {
                                    temp.title = i2b2.CRC.ctrlr.MQryStatusPatient._GetTitle(temp.QRS_Type, temp, qi);
                                }
        ...
        private_refreshInterrupt = setInterval("i2b2.CRC.ctrlr.currentQueryStatus.patientSetrefreshStatus()", 100);
    ...
    return {
        ...
        running: false,
        started: false,
        patientSetstartQuery: function(queryName, ajaxParams) {
            this.name = queryName;
            this.params = ajaxParams;
            patientSetPrivate_startQuery.call(this);
        },
        ...
        patientSetrefreshStatus: function() {
            patientSet_private_refresh_status();
        },
        pollStatus: function() {
            private_pollStatus();
        }
