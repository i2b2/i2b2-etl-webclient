...
function i2b2_BaseCell(configObj) {
	...
	this.Init = (function(inURL, inParams) {
		...
		if (configObj.code === 'CRC') {
			if (env.etl === true) {
				const etlFile = ["ETL/CRC_view_Etl.js"];
				this.cfg.files.push(...etlFile);
			}
			if (env.tabulation === true) {
				const tabulationFile = ["Tabulation/CRC_view_Tabulation.js"];
				this.cfg.files.push(...tabulationFile);
			}
			if (env.d3 === true) {
				const d3File = ["D3/CRC_view_D3.js"];
				this.cfg.files.push(...d3File);
			}
			if (env.facts === true) {
				const factsFile = ["Facts/CRC_view_Facts.js"];
				this.cfg.files.push(...factsFile);
			}
			if (env.factsAll === true) {
				const factsAllFile = ["Facts/CRC_view_AllFacts.js"];
				this.cfg.files.push(...factsAllFile);
			}
			if (env.sankey === true) {
				const sankeyFile = ["Sankey/CRC_view_Sankey.js"];
				this.cfg.files.push(...sankeyFile);
			}
			if (env.computeDerivedFact === true) {
				const computeDerivedFactFiles = ["CRC_Common/CRC_ctrlr_MComputeFact.js"];
				this.cfg.files.push(...computeDerivedFactFiles);
			} 
			if (env.derivedConcept === true && env.derivedConceptTemporal === true) {
		...
		if (configObj.code === 'ONT') {
			if (env.derivedConcept === true && env.derivedConceptTemporal === true) {
				const derivedConceptFiles = ["ONT_Common/ONT_sdx_MConcept.js", "ONT_Common/ONT_ctrlr_MGeneral.js"];
