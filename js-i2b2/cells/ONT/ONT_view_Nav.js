...
i2b2.ONT.view.nav.PopulateCategories = function() {		
	...
	for (var i=0; i<i2b2.ONT.model.Categories.length; i++) {
		...
		if (env.derivedConcept === true) {
			i2b2.ONT.view.mNav.PCategories(sdxDataNode);
		}
		//===========Commented for next-release===========//
	...
	if (tvNode) {
		if (tvNode.data.i2b2_SDX) {
			if (tvNode.data.i2b2_SDX.sdxInfo.sdxType == "CONCPT") {
				...
				if (env.derivedConcept === true) {
					let contextRecord = i2b2.ONT.view.nav.contextRecord;
					i2b2.ONT.view.mNav.CMenuValidate(contextRecord, tvNode, mil);
				}
				if (env.createMLPrediction === true) {
					let contextRecord = i2b2.ONT.view.nav.contextRecord;
					i2b2.ONT.view.mNav.CreateMLPrediction(contextRecord, tvNode, mil);
				}
				//===========Commented for next-release===========//
