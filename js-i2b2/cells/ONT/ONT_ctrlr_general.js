...
i2b2.ONT.ctrlr.gen.loadCategories = function() {
	...
	var processXML = function(i2b2CellMsg) {
		...
		if (!i2b2CellMsg.error) {		
			...
			for(var i=0; i<1*c.length; i++) {
				...
				o.isDerived = false; // added for Derived-Concept
				o.derivedId = null;
				o.derivedCode = '';
				this.push(o);
			}
	...
	if (i2b2.PM.model.userRoles.includes('POPULATION_FACT_VIEWER') && (env.derivedConcept === true)) {
		i2b2.ONT.ctrlr.MGen.getDerivedConcept(i2b2.ONT.model.Categories);
	}
	//===========Commented for M-release===========//
