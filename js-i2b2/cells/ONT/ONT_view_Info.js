...
//================================================================================================== //
i2b2.ONT.view.info.SetKey = function(key, dCode) { 
	...
	} else if (hasClass(factsTab, 'active')) {
		i2b2.CRC.view.facts.clickFacts(keyEncode);
	} else if (hasClass(sankeyTab, 'active')) {
		i2b2.CRC.view.sankey.getSankeyByFacts(keyEncode);
	} else if (hasClass(d3Tab, 'active')) {
		document.getElementById("droppedDiv").style.zIndex = 0;
		document.getElementById("zoom_btns").classList.add("active_btn");
		if (decodeURI(keyEncode).includes('Sankey')) {
			i2b2.CRC.view.D3.getD3ByConceptPath(keyEncode);
		} else {
			return;
		}
	}
